...
use docx_rs::read_docx_from_xml;

fn main() {
    // Read the provided XML file
    let xml_content = fs::read_to_string("/Users/bokuweb/Downloads/ooxml.xml")
        .expect("Failed to read XML file");
    
    println!("Reading XML file with length: {} characters", xml_content.len());
    
    match read_docx_from_xml(&xml_content) {
        Ok(docx) => {
